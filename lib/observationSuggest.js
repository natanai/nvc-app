...
export async function loadCueLibrary(
  ...
  const sourceUrl = typeof primaryUrl === 'string' && primaryUrl
    ? primaryUrl
    : '/data/observation_cue_library.json';

  const prefersCsv = /\.csv(?:$|\?)/i.test(sourceUrl);

  if (!prefersCsv) {
    const dataset = await fetchCueLibraryDataset(sourceUrl);
    if (dataset) {
      return dataset;
    }
  }

  const csvSource = prefersCsv ? sourceUrl : '/data/observation_cues.sanitized.csv';
  return fetchCueLibraryFromCsv(csvSource, modulesUrl);
}

async function fetchCueLibraryDataset(url) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      return normalizeCueLibraryDataset(json);
    }
    ...
    try {
      ...
      console.warn('Cue library dataset fetch returned non-JSON payload. Falling back to CSV.', error);
      return null;
    }
  } catch (error) {
    console.warn('Unable to load cue library dataset. Falling back to CSV.', error);
    return null;
  }
}

async function fetchCueLibraryFromCsv(csvUrl, modulesUrl) {
  try {
    ...
    console.warn('Unable to load cue library from CSV.', error);
    return { cues: [], modules: [], slotIndex: {} };
  }
}

function normalizeCueLibraryDataset(json) {
  if (!json || typeof json !== 'object') {
    return null;
  }
  const cues = Array.isArray(json.cues)
    ? json.cues.map(deserializeCueLibraryCue).filter(Boolean)
    : [];
  const modules = Array.isArray(json.modules)
    ? json.modules.map(deserializeCueLibraryModule).filter(Boolean)
    : [];
  if (!cues.length && !modules.length) {
    return null;
  }
  const slotIndex = normalizeCueLibrarySlotIndex(json.slotIndex);
  return { cues, modules, slotIndex };
}

function normalizeCueLibrarySlotIndex(index) {
  ...
  Object.keys(index).forEach(key => {
    ...
    normalized[key] = Array.isArray(value) ? value.slice() : [];
  });
  return normalized;
}

function deserializeCueLibraryCue(entry) {
  ...
  return {
    ...
    phrase: entry.phrase,
    phrases: Array.isArray(entry.phrases) ? entry.phrases.slice() : [],
    feelings: Array.isArray(entry.feelings) ? entry.feelings.slice() : [],
    needs: Array.isArray(entry.needs) ? entry.needs.slice() : [],
    slotCoverage: Array.isArray(entry.slotCoverage) ? entry.slotCoverage.slice() : [],
    slotSummary: entry.slotSummary,
    moduleId: typeof entry.moduleId === 'string' ? entry.moduleId : '',
    patterns: Array.isArray(entry.patterns)
      ? entry.patterns.map(deserializeRegex).filter(Boolean)
      : [],
    matchers: Array.isArray(entry.matchers)
      ? entry.matchers.map(deserializeCueLibraryMatcher).filter(Boolean)
      : [],
  };
}

function deserializeCueLibraryModule(entry) {
  if (!entry || typeof entry !== 'object') {
    return null;
  }
  return {
    ...
    slotIds: Array.isArray(entry.slotIds) ? entry.slotIds.slice() : [],
    slotSummary: entry.slotSummary,
    cueIds: Array.isArray(entry.cueIds) ? entry.cueIds.slice() : [],
    feelings: Array.isArray(entry.feelings) ? entry.feelings.slice() : [],
    needs: Array.isArray(entry.needs) ? entry.needs.slice() : [],
    examples: Array.isArray(entry.examples) ? entry.examples.slice() : [],
    guidance: Array.isArray(entry.guidance) ? entry.guidance.slice() : [],
    matchers: Array.isArray(entry.matchers)
      ? entry.matchers.map(deserializeCueLibraryMatcher).filter(Boolean)
      : [],
  };
...
function deserializeCueLibraryMatcher(entry) {
  if (!entry || typeof entry !== 'object') {
    return null;
  }
  const regex = deserializeRegex(entry.regex);
  if (!regex) {
    return null;
  }
  const tokens = Array.isArray(entry.tokens) ? entry.tokens.slice() : [];
  const tokenSetValues = Array.isArray(entry.tokenSet) ? entry.tokenSet : [];
  return {
    key: typeof entry.key === 'string' ? entry.key : '',
    regex,
    tokens,
    tokenSet: new Set(tokenSetValues),
    tokenThreshold: Number.isFinite(entry.tokenThreshold) ? entry.tokenThreshold : 0,
    sourceType: typeof entry.sourceType === 'string' ? entry.sourceType : '',
    sources: Array.isArray(entry.sources) ? entry.sources.slice() : [],
  };
}

function deserializeRegex(entry) {
  if (!entry) {
    return null;
  }
  ...
  if (typeof entry === 'string') {
    try {
      ...
      console.warn('Unable to deserialize regex string', entry, error);
      return null;
    }
  }
  if (typeof entry === 'object' && typeof entry.source === 'string') {
    const flags = typeof entry.flags === 'string' ? entry.flags : '';
    try {
      return new RegExp(entry.source, flags);
    } catch (error) {
      console.warn('Unable to deserialize regex source', entry, error);
      return null;
    }
  }
  return null;
