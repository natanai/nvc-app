...
export function suggestFromObservation(text, source, maxEach = 6, options = {}) {
  ...
  const normalizedMax = Number.isFinite(maxEach) ? Math.max(maxEach, 0) : 0;
  const { needs, feelings } = selectNeedsAndFeelings(limitedHits, {
    maxNeeds: Math.min(normalizedMax || 4, 4),
    maxFeelings: Math.min(normalizedMax || 4, 4),
  });
...
function selectNeedsAndFeelings(hits, { maxNeeds = 4, maxFeelings = 4 } = {}) {
  const scoredHits = Array.isArray(hits) ? hits : [];
  const needs = selectTopNeeds(scoredHits, maxNeeds);
  const feelings = selectTopFeelings(scoredHits, needs, maxFeelings);
  return { needs, feelings };
}

function selectTopNeeds(hits, maxNeeds) {
  if (!maxNeeds) {
    return [];
  }
  const stats = new Map();
  hits.forEach((hit, hitIndex) => {
    ...
    needs.forEach((need, needIndex) => {
      const normalized = typeof need === 'string' ? need.trim() : '';
      if (!normalized) {
        return;
      }
      ...
      existing.count += 1;
      existing.totalScore += Number.isFinite(hit?.score) ? hit.score : 0;
      if (hitIndex < existing.firstHitIndex) {
        existing.firstHitIndex = hitIndex;
      }
  ...
  const ranked = Array.from(stats.values()).sort((a, b) => {
    if (a.firstHitIndex !== b.firstHitIndex) {
  ...
  return ranked.slice(0, Math.max(maxNeeds, 0)).map(entry => entry.need);
}

function selectTopFeelings(hits, selectedNeeds, maxFeelings) {
  if (!maxFeelings) {
    return [];
  }
  const needSet = new Set(Array.isArray(selectedNeeds) ? selectedNeeds.filter(Boolean) : []);
  if (!needSet.size) {
    return uniqueStrings(hits.flatMap(hit => hit?.module?.feelings || [])).slice(0, maxFeelings);
  }

  const stats = new Map();
  hits.forEach((hit, hitIndex) => {
    const moduleNeeds = Array.isArray(hit?.module?.needs) ? hit.module.needs : [];
    const moduleFeelings = Array.isArray(hit?.module?.feelings) ? hit.module.feelings : [];
    const matchingNeeds = moduleNeeds.filter(need => needSet.has(need));
    if (!matchingNeeds.length || !moduleFeelings.length) {
      return;
    }
    moduleFeelings.forEach((feeling, feelingIndex) => {
      const normalized = typeof feeling === 'string' ? feeling.trim() : '';
      if (!normalized) {
        return;
      }
      const existing = stats.get(normalized) || {
      ...
      existing.count += 1;
      existing.totalScore += Number.isFinite(hit?.score) ? hit.score : 0;
      matchingNeeds.forEach(need => existing.needMatches.add(need));
      if (hitIndex < existing.firstHitIndex) {
        existing.firstHitIndex = hitIndex;
      }
  ...
  const ranked = Array.from(stats.values()).sort((a, b) => {
    ...
    if (b.needMatches.size !== a.needMatches.size) {
      return b.needMatches.size - a.needMatches.size;
    }
    if (b.totalScore !== a.totalScore) {
      return b.totalScore - a.totalScore;
    }
  ...
  return ranked.slice(0, Math.max(maxFeelings, 0)).map(entry => entry.feeling);
}
