...
export function scaffoldRewrite(input = {}) {
  const parts = [];
  const when = normalizeAnchorSegment(input.when);
  if (when) {
    parts.push(when);
  }
  const where = normalizeAnchorSegment(input.what);
  if (where) {
    parts.push(where);
  }
  ...
  const sentence = ensureSentenceTermination(core);
  const hope = buildHopeSentence(input.gap);
  return hope ? sentence + hope : sentence;
}

function normalizeAnchorSegment(value) {
...
function ensureSentenceTermination(text) {
...
function buildHopeSentence(raw) {
  const cleaned = normalizeAnchorSegment(raw);
  if (!cleaned) {
    return '';
  }
  const stripped = stripTrailingSentencePunctuation(cleaned);
  if (!stripped) {
    return '';
  }
  return ` I had hoped ${stripped}.`;
}
