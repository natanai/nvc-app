...
const OBSERVATION_POSITIVE_PATTERNS = [
  ...
  },
  {
    ...
    message: 'Naming who was involved keeps the observation grounded.',
    regex:
      /\b(?:he|she|they|them|him|her|we|us|you|someone|somebody|anyone|anybody|everyone|everybody|staff|team|group|coworker|colleague|manager|supervisor|client|customers?|participant|participants|student|students|teacher|teachers|parent|parents|child|children|neighbor|neighbour|friend|friends|guest|guests|facilitator|speaker|host)\b/gi,
  },
];
...
function collectObservationHighlights(text, protectedRanges) {
  ...
  OBSERVATION_POSITIVE_PATTERNS.forEach(pattern => {
    ...
    matches.forEach(hit => {
      ...
      seen.add(key);
      const slotId = typeof pattern.slotId === 'string' ? pattern.slotId.trim() : '';
      const keyName = slotId || pattern.key || '';
      entries.push({
        token,
        label: pattern.label || '',
