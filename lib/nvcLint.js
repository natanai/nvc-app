...
const OBSERVATION_POSITIVE_PATTERNS = [
  { label: 'time anchors', regex: /\b(?:yesterday|today|tonight|this morning|this afternoon|this evening|last night|last week|last weekend|last month|last year|earlier today|earlier this week|earlier tonight|later that day|later that night)\b/gi },
  { label: 'weekday anchors', regex: /\bon\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/gi },
  { label: 'calendar anchors', regex: /\bon\s+\d{1,2}(?:st|nd|rd|th)?\s+(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+\d{2,4})?\b/gi },
  { label: 'numeric dates', regex: /\bon\s+\d{1,2}\/\d{1,2}(?:\/\d{2,4})?\b/gi },
  { label: 'clock times', regex: /\b(?:at|around)\s+\d{1,2}(?::\d{2})?\s?(?:a\.?m\.?|p\.?m\.?|am|pm)\b/gi },
  { label: 'time of day words', regex: /\b(?:at\s+(?:noon|midnight)|that\s+morning|that\s+evening|that\s+afternoon|sunrise|sunset)\b/gi },
  { label: 'counts and measures', regex: /\b(?:once|twice|thrice|three times|four times|five times|six times|seven times|eight times|nine times|ten times|\d+[\d,:]*\s?(?:times?|minutes?|hours?|days?|weeks?|months?|years?|emails?|messages?|calls?|people|instances?|occasions?))\b/gi },
  { label: 'sensory verbs', regex: /\b(?:i|we)\s+(?:saw|see|heard|hear|notice|noticed|observe|observed|witness|witnessed|recorded|captured|measured|timed|counted|checked|read|watched)\b/gi },
  { label: 'when anchors', regex: /\bwhen\s+(?:i|we|you|they)\s+(?:saw|see|heard|hear|noticed|observe|observed)\b/gi },
  {
    label: 'location anchors',
    regex:
      /\b(?:at|in|inside|outside|on|by|near)\s+(?:the\s+)?(?:room\s+\d+[a-z]?|classroom\s+\d+[a-z]?|office\s+\d+[a-z]?|conference\s+room\s+\d+[a-z]?|meeting\s+room\s+\d+[a-z]?|suite\s+\d+[a-z]?|desk\s+\d+[a-z]?|building\s+[a-z0-9]+|floor\s+\d+|wing\s+[a-z0-9]+|section\s+[a-z0-9]+|aisle\s+\d+|cafeteria|library|kitchen|lobby|hallway|auditorium|gym|lab|laboratory|parking\s+lot|front\s+desk|reception\s+desk|entrance|exit|warehouse|break\s+room|restroom|bathroom|playground|field|court|studio|workshop|workspace|yard)\b/gi,
  },
  {
    label: 'named locations',
    regex: /\b(?:at|in|inside|outside|on|by|near)\s+(?:the\s+)?(?:[A-Z][\w&]*\s+[A-Z0-9][\w&]*(?:\s+[A-Z0-9][\w&]*)*)\b/g,
  },
  { label: 'direct quotes', regex: /“[^”]+”|"[^"\n]+"|‘[^’]+’|\'[^'\n]+\'/g, skipProtected: true },
];
...
function collectPatternMatches(text, patterns, protectedRanges) {
  ...
  patterns.forEach(item => {
    ...
    hits.forEach(hit => {
      ...
      if (isRangeProtected(hit.start, hit.end, protectedRanges)) {
        return;
      }
