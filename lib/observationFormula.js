...
const CLOCK_TIME_REGEX = /\b(?:at|around|by)\s+\d{1,2}(?::\d{2})?\s?(?:a\.?m\.?|p\.?m\.?|am|pm)\b/gi;
...
const LOCATION_VIRTUAL_REGEX = new RegExp(
  `\\b(?:on|over)\\s+(?:${buildWordAlternation(LOCATION_VIRTUAL_KEYWORDS)})\\b`,
  'gi',
);
