...
1. **Automated final-head gate — complete for the functional audit:** Site Quality run 697 passed the complete committed-site and authoring suite, the canonical build was run twice locally with the same zero-diff hash, and the published functional tree matched the verified local tree. Repeat this gate only if a later commit changes functional, generated, or fact-checking files.
