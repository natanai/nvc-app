...
- Core content: `Needs.csv`, `Feelings.csv`, `Faux Feelings.csv`, `Strategies.csv`, `poems.csv`
...
- Visual metadata stays in place: `data/color-palettes.csv` remains outside the export because it is purely stylistic, not factual.
...
| Feeling poem quotes | `data/poems_formatted.txt` (edit via `fact-checking/poems.csv`) | Quotes and URLs shown on feeling pages. | `npm run build:data && npm run build:pages` |
...
| Visual metadata | `data/color-palettes.csv`, `data/body-regions.json` | Palette values for magnets and body region labels. Palettes are visual-only and do not export to `fact-checking/`. | `npm run build:data && npm run build:pages` |
