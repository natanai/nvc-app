...
Permanent coverage lives in `tests/shared-density-polish.test.mjs`, including parser discovery, full-width phone ownership, grouped Quick check rows, disclosure treatment, minimum touch target sizing, the overlay paint/scroll synchronization contract, and the ban on `!important` patch chains.
