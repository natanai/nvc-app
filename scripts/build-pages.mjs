      ...
      <script>
        (function() {
          ...
          for (var i = 0; i < storages.length; i += 1) {
            ...
            try {
              ...
              if (typeof candidate === 'string') {
                ...
                if (trimmed) {
                  candidates.push({ value: trimmed, index: candidates.length });
                }
              }
            } catch (error) {}
          }
          ...
          for (var c = 0; c < candidates.length; c += 1) {
            ...
            candidate.timestamp = parseUpdatedAt(candidate.value);
            candidate.rankIndex = typeof candidate.index === 'number' ? candidate.index : c;
          }

          var remainingCandidates = candidates.slice();
          var parsed = null;

          while (remainingCandidates.length) {
            var bestIndex = 0;
            var bestCandidate = remainingCandidates[0];

            for (var r = 1; r < remainingCandidates.length; r += 1) {
              var contender = remainingCandidates[r];
              if (
                contender.timestamp > bestCandidate.timestamp ||
                (contender.timestamp === bestCandidate.timestamp && contender.rankIndex < bestCandidate.rankIndex)
              ) {
            ...
            try {
              var parsedCandidate = JSON.parse(bestCandidate.value);
