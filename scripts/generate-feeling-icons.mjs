...
const pillIconsDir = path.join(rootDir, 'icons', 'feelings', 'pills');
...
function buildSvg(slug) {
  const body = buildSvgBody(slug);
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none" role="img" aria-hidden="true">${body}</svg>\n`;
}

function buildPillSvg(slug) {
  const body = buildSvgBody(slug);
  const width = 320;
  const height = 120;
  const radius = height / 2;
  const clipId = `pill-clip-${slug}`;
  const sourceSize = 64;
  const scale = Math.max(width / sourceSize, height / sourceSize);
  const offsetX = (width - sourceSize * scale) / 2;
  const offsetY = (height - sourceSize * scale) / 2;

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" fill="none" role="img" aria-hidden="true">`
    + `<defs><clipPath id="${clipId}"><rect x="0" y="0" width="${width}" height="${height}" rx="${radius}" /></clipPath></defs>`
    + `<g clip-path="url(#${clipId})"><rect width="${width}" height="${height}" fill="none" />`
    + `<g transform="translate(${offsetX.toFixed(4)} ${offsetY.toFixed(4)}) scale(${scale.toFixed(6)})">${body}</g>`
    + `</g></svg>\n`;
}
...
async function writePillIcons(slugs) {
  await ensureDirectory(pillIconsDir);
  await Promise.all(
    slugs.map(async (slug) => {
      const svg = buildPillSvg(slug);
      const filePath = path.join(pillIconsDir, `${slug}.svg`);
      await fs.writeFile(filePath, svg, 'utf8');
    })
  );
}
...
async function writeCss(slugs) {
  ...
  const rules = slugs
    .map((slug) => `.magnet[data-magnet-id='feelings-${slug}'] {\n  --magnet-icon: url('icons/feelings/pills/${slug}.svg');\n}\n`)
    .join('\n');
